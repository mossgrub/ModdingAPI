...
namespace Modding
{
    ...
    public abstract class Mod : Loggable, IMod
    {
        ...
        private readonly MethodInfo onLoadGlobalSettings;
        private readonly MethodInfo onSaveGlobalSettings;
        private readonly Type saveSettingsType = null;
        private readonly MethodInfo onLoadSaveSettings;
        private readonly MethodInfo onSaveSaveSettings;
                ...
                foreach (var mi in globalType.GetMethods())
                {
                    switch (mi.Name)
                    {
                        case nameof(IGlobalSettings<object>.OnLoadGlobal):
                            this.onLoadGlobalSettings = mi;
                            break;
                        case nameof(IGlobalSettings<object>.OnSaveGlobal):
                            this.onSaveGlobalSettings = mi;
                            break;
                    }
                }
            }
                ...
                foreach (var mi in saveType.GetMethods())
                {
                    switch (mi.Name)
                    {
                        case nameof(ILocalSettings<object>.OnLoadLocal):
                            this.onLoadSaveSettings = mi;
                            break;
                        case nameof(ILocalSettings<object>.OnSaveLocal):
                            this.onSaveSaveSettings = mi;
                            break;
                    }
                }
            }
        ...
        private void HookSaveMethods()
        {
            ...
            UnityEngine.Application.focusChanged += focus => { if (!focus) SaveGlobalSettings(); };
        }
        ...
        private void SceneChanged(Scene arg0, Scene arg1)
        {
            ...
            {
                this.onLoadSaveSettings.Invoke(this, new object[] { Activator.CreateInstance(saveType) });
            }
        }
        ...
        private bool TryLoadGlobalSettings(string path, Type saveType)
        {
            ...
            this.onLoadGlobalSettings.Invoke(this, new[] { obj });
            return true;
        }
        ...
        protected void SaveGlobalSettings()
        {
            try
            {
                if (this.globalSettingsType is Type saveType)
                {
                    ...
                    var obj = this.onSaveGlobalSettings.Invoke(this, null);
                    if (obj is null)
                        return;
        ...
        private void LoadLocalSettings(ModSavegameData data)
        {
            try
            {
                ...
                this.onLoadSaveSettings.Invoke
                (
                    this,
                    new[]
                    {
        ...
        private void SaveLocalSettings(ModSavegameData data)
        {
            try
            {
                ...
                var settings = this.onSaveSaveSettings.Invoke(this, null);
